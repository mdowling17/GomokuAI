...
def mcts(state, num_rollouts=NUM_ROLLOUTS):
    ...
    """
    root = Node(state)
    # turn is 225 - number of empty spaces
    turn = 225 - state.board[gm.EMPTY].sum()
    if turn < 10:
        num_rollouts = 255
    elif turn < 20:
        num_rollouts = 1000
    else:
        num_rollouts = 1000 * (turn // 10)
...
if __name__ == "__main__":
    ...
    # ========================================
    # === Test for 3 in a row horizontally ===
    # ========================================

    state = gm.GomokuState.blank(15, 5)
    state = state.play_seq([(0,1), (1,1), (0,2), (1,2), (0,3)])
    print(state.corr[0])
    print("✅ no fails")